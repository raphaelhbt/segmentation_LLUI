...
def predict(net, inputs, targets):
	predictions = net(inputs)
	predictions = torch.sigmoid(predictions)

	# Compute the Dice score
	score=dice_metric((predictions>0.5).float(), targets)     
	return score
...
with torch.no_grad():
    ...
    for i, (inputs, targets) in enumerate(test_loader):
        ...
        total_score.append(outputs)
        print(f"Batch {i+1}/{len(test_loader)} - Dice score: {outputs}")
    print(f"Average Dice score: {sum(total_score)/len(total_score)}")

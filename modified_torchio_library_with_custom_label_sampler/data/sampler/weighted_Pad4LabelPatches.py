...
class WeightedSampler_Pad4LabelPatches(RandomSampler_Pad4LabelPatches):
    ...
    def clear_probability_borders(
		...
		# ADAPTATION JOSUA: it does not set the proabability map to 0
		#                   at the border if the value at this poisition is equal to the max probability
		#                   of the probabaility map
...
#ADAPTED Josua -------
        ...
        # Apply initial cropping with condition
        probability_map[:crop_i, :, :] = np.where(probability_map[:crop_i, :, :] != max_value, 0, probability_map[:crop_i, :, :])
        probability_map[:, :crop_j, :] = np.where(probability_map[:, :crop_j, :] != max_value, 0, probability_map[:, :crop_j, :])
        probability_map[:, :, :crop_k] = np.where(probability_map[:, :, :crop_k] != max_value, 0, probability_map[:, :, :crop_k])



	# Convert crop_fin to list
...
# ---------------------------
    ...
    def get_random_index_ini(
        ...
        # assert np.all(index_ini >= 0) # ADAPTED JOSUA (commented)

...
class PatchSampler_Pad4LabelPatches:
    ...
    def _get_crop_transform(
        ...
        from ...transforms.preprocessing.spatial.pad import Pad    #JOSUA adapted
        from ...transforms.augmentation.composition import Compose # JOSUA adapted
        
        shape = np.array(subject.spatial_shape, dtype=np.uint16)
        index_ini_array = np.array(index_ini, dtype=np.int16)
...
#ADAPTED Josua -------
        ...
        pad_ini = np.maximum(-index_ini_array, 0)
        pad_fin = np.maximum(index_fin - shape, 0)
        pad_values = list(sum(zip(pad_ini, pad_fin), ()))

	
        # Calculate cropping values
        crop_ini = np.maximum(index_ini_array, 0).tolist()
        crop_fin = np.maximum(shape - np.minimum(index_fin, shape), 0).tolist()
        cropping = list(sum(zip(crop_ini, crop_fin), ()))


        if any(pad_values):
            # Return combined Pad and Crop transformation
            return Compose([Pad(pad_values), Crop(cropping)])
        else:
...
class RandomSampler_Pad4LabelPatches(PatchSampler_Pad4LabelPatches):
